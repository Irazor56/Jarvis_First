...
class Music(commands.Cog):
    ...
    async def join(
        ...
        destination = (
            channel if channel else ctx.author.voice.channel
        )  # CHOOSING THE DESTINATION, MIGHT BE THE REQUESTED ONE, BUT IF NOT THEN WE PICK AUTHORS VOICE CHANNEL

        if ctx.voice_client:  # CHECKING IF THE BOT IS PLAYING SOMETHING
            await ctx.VoiceState.voice.move_to(
                destination
...
bot = commands.Bot(
    ...
    description="Relatively simple bot",
)
